...
# Simplify ZSH PROMPT
echo 'PROMPT="${debian_chroot:+($debian_chroot)─}${VIRTUAL_ENV:+($(basename $VIRTUAL_ENV))─}%B%F{%(#.red.green)}%n@%m%b%F{%(#.green.cyan)}-[%B%F{reset}%(6~.%-1~/…/%4~.%5~)%b%F{%(#.green.cyan)}]%F{%(#.red.reset)}%#%F{reset} "' >> $HOME/.zshrc
